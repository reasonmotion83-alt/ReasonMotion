...
"""
ReasonMotion Offline Quantitative Evaluation Script (Offline Evaluator)
This script loads the specified checkpoint epoch and performs quantitative metric evaluation on the test set (split=2).
...
def get_target_checkpoint(args, ckpt_dir):
    ...
    # 1. First try to match the --step argument (for RL step counts)
    ...
    # 2. Fall back to matching the --epoch argument
...
def main():
    # Pre-parse the --config / -c argument
    ...
    if config_yaml_path and os.path.exists(config_yaml_path):
        try:
            ...
            print(f"⚠️ Failed to read config file ({config_yaml_path}): {e}")
    ...
    if not args.exp_dir:
        parser.error("Must specify --exp_dir or set exp_dir in configs/eval.yaml.")

    args.exp_dir = os.path.abspath(args.exp_dir)

    config_path = os.path.join(args.exp_dir, ".hydra", "config.yaml")
    if not os.path.exists(config_path):
        raise FileNotFoundError(f"Hydra config file not found at: {config_path}")
    ...
    try:
        ...
        print("📂 Loading test dataset (Split 2)...")
        if args.quick:
        ...
        if 0.0 < args.sample_ratio < 1.0:
            ...
            print(f"✂️ Applying data ratio --sample_ratio {args.sample_ratio:.2f}: selecting {num_samples} samples out of {len(test_dataset)} for evaluation.")
        ...
        print(f"🚀 Starting quantitative metric evaluation (n_sample={args.nsample}, total samples={len(test_dataset)})...")
        ...
        if eval_dir and filename_stem and json_path:
            ...
            print(f"💾 Metrics JSON automatically saved to: {json_path}")
            print(f"💾 Evaluation log automatically saved to: {log_path}\n")
