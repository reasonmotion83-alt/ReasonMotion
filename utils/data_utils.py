...
def expmap2rotmat_torch(r):
    """
    Converts exponential map to rotation matrix.
    Batch PyTorch version.
    :param r: N x 3
    :return: N x 3 x 3
    """
    ...
    n = r1.shape[0]
    eye = torch.eye(3, 3, device=device).repeat(n, 1, 1).float()
    sin_term = torch.sin(theta).unsqueeze(1).repeat(1, 9).view(-1, 3, 3)
    cos_term = (1 - torch.cos(theta)).unsqueeze(1).repeat(1, 9).view(-1, 3, 3)

    R = eye + sin_term * r1 + cos_term * torch.matmul(r1, r1)
    return R
