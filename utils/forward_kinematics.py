...
def fkl_torch(angles, parent, offset, rotInd, expmapInd):
    ...
    offset_torch = torch.tensor(offset, dtype=torch.float32, device=device)
    p3d = offset_torch.unsqueeze(0).repeat(n, 1, 1)
    ...
    for i in np.arange(1, j_n):
        if parent[i] > 0:
            ...
            p3d_i = p3d[0, i, :].unsqueeze(0).unsqueeze(1).expand(n, 1, 3)
            transformed = torch.bmm(p3d_i, R[:, parent[i], :, :]).squeeze(1)
            p3d[:, i, :] = transformed + p3d[:, parent[i], :]
    return p3d
